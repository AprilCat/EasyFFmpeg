...
static int h264_extradata_to_annexb(H264BitStreamFilterContext *ctx, 
    uint8_t *ext_extradata, int32_t ext_extradata_size, const int padding)
{
    uint16_t unit_size;
    uint64_t total_size = 0;
    uint8_t *out = NULL, *new_out = NULL, unit_nb, sps_done = 0,
        sps_seen = 0, pps_seen = 0;
    const uint8_t *extradata = ext_extradata + 4;
    ...
    while (unit_nb--) {
        ...
        if (extradata + 2 + unit_size > ext_extradata + ext_extradata_size) {
            av_log(NULL, AV_LOG_ERROR, "Packet header is not contained in global extradata, "
                "corrupted stream or invalid MP4/AVCC bitstream\n");
            free(out);
            return AVERROR(EINVAL);
        }
        if ((new_out = (uint8_t*)realloc(out, total_size + padding)) == 0) {
            free(out);
            err = AVERROR(ENOMEM);
            return err;
        }
    ...
    pps:
        if (!unit_nb && !sps_done++) {
            ...
            if (unit_nb) {
                ctx->pps_offset = (extradata - 1) - (ext_extradata + 4);
                pps_seen = 1;
            }
        }
    }
    ...
    uint8_t **poutbuf, int *poutbuf_size, int *poutbuf_capacity, const uint8_t *buf, int buf_size)
{
    ...
    *poutbuf_size = 0;
    ...
    if (!poutbuf || !poutbuf_size || !poutbuf_capacity) {
        return -1;
    }

    /* nothing to filter */
    if (!(ext_extradata) || ext_extradata_size < 6) {
        *got_output = 0;
        return 0;
    }
    ...
    if (!ctx->extradata_parsed) {
        ret = h264_extradata_to_annexb(ctx, ext_extradata, ext_extradata_size, AV_INPUT_BUFFER_PADDING_SIZE);
        if (ret < 0)
            return ret;
        ctx->length_size = ret;
        ctx->new_idr = 1;
    ...
    do {
        ...
        if (unit_type == 7)
            ...
            if (!ctx->idr_sps_seen) {
                if (ctx->sps_offset == -1)
                    av_log(NULL, AV_LOG_WARNING, "SPS not present in the stream, nor in AVCC, stream may be unreadable\n");
                else {
                    if ((ret = alloc_and_copy(poutbuf, poutbuf_size, poutbuf_capacity,
        ...
        * This could be checking idr_pic_id instead, but would complexify the parsing. */
        ...
        if (ctx->new_idr && unit_type == 5 && !ctx->idr_sps_seen && !ctx->idr_pps_seen) {
            ...
            if (ctx->pps_offset == -1) {
                av_log(NULL, AV_LOG_WARNING, "PPS not present in the stream, nor in AVCC, stream may be unreadable\n");
                if ((ret = alloc_and_copy(poutbuf, poutbuf_size, poutbuf_capacity,
                    NULL, 0, buf, nal_size)) < 0)
                    goto fail;
            }
            else if ((ret = alloc_and_copy(poutbuf, poutbuf_size, poutbuf_capacity,
